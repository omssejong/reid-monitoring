...
type StorageJob struct {
	...
	Mode   string `json:"mode"`   // "percent" | "date"
...
func (s *StorageJobStore) runDate(ctx context.Context, job *StorageJob, from, to time.Time) {
	...
	for _, c := range filtered {
		...
		if loopCount%storageDeleteLogEvery == 0 {
			log.Info(fmt.Sprintf("storage job date progress: id=%s processed=%d/%d deleted=%d",
				job.ID, loopCount, len(filtered), job.DeletedCount))
		}
	}
}
